...
# loading in data
d <- list.files(path = "./Mousetracking_experiment/data/", pattern = "*.csv", full.names = T) %>% 
  map_df(~read_csv(.))
...
d5 <- d %>% filter(subject_nr == 5) %>% filter(count_logger==1)
```
...
```{r}
mt_plot(m, color = 'congruent', alpha = 0.3)
```
